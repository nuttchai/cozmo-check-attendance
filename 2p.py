...
def notifyPicture(url,robot: cozmo.robot.Robot):
    ...
    if face_detect(url) == 1:
        if Absent == 1:
            ...
            robot.say_text(date() + day() +"There are " + str(face_detect(url)) + " student in the classroom and 1 student who's absent.").wait_for_completed()
        elif Absent > 1:
            payload = {'message': "   Present: " + str(face_detect(url)) + "     Absent: " + str(Absent)}
            robot.say_text(date() + day() +"There are " + str(face_detect(url)) + " student in the classroom and " + str(Absent) + " student who's absent.").wait_for_completed()
        else:
            payload = {'message': "   Present: " + str(face_detect(url)) + "     Absent: " + str(Absent)}
            robot.say_text(date() + day() +"There are " + str(face_detect(url)) + " student in the classroom and no student who's absent.").wait_for_completed()
...
def score_attendace(a, result):
    all_student = ['61011300', '61011318', '61011359']
    attendance = []
