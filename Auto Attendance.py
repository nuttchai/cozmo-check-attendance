...
all_student_id = ['61011300', '61011318', '61011359'] #EDIT ID Student HERE
Total_Student = 3 #EDIT Number of student in classroom
path = Pic_Path
...
def notifyPicture(url,robot: cozmo.robot.Robot):
    ...
    S = Total_Student
    Absent = S - len(student_id())
    
    if len(student_id()) == 1:
        if Absent == 1:
            payload = {'message': "   Present: " + str(len(student_id())) + "     Absent: 1"}
            robot.say_text(date() + day() +"There are " + str(len(student_id())) + " student in the classroom and 1 student who's absent.").wait_for_completed()
        elif Absent > 1:
            payload = {'message': "   Present: " + str(len(student_id())) + "     Absent: " + str(Absent)}
            robot.say_text(date() + day() +"There are " + str(len(student_id())) + " student in the classroom and " + str(Absent) + " student who's absent.").wait_for_completed()
        else:
            payload = {'message': "   Present: " + str(len(student_id())) + "     Absent: " + str(Absent)}
            robot.say_text(date() + day() +"There are " + str(len(student_id())) + " student in the classroom and no student who's absent.").wait_for_completed()

    elif len(student_id()) == 0:
        payload = {'message': "   Present: " + str(len(student_id())) + "     Absent: " + str(Absent)}
        robot.say_text(date() + day() +"There is no student in the classroom.").wait_for_completed()
...
def student_id():
    ...
    for unknown_face_encoded in unknown_faces_encoded:
        matches = face_recognition.compare_faces(known_faces, unknown_face_encoded, tolerance=0.5)
        if True in matches:
